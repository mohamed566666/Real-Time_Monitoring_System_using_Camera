...
class VerifyUserIdentityUseCase:
    """Verify if the provided embeddings match the stored embeddings for a username"""
    ...
    def find_best_match(
        ...
        """Find the best matching embedding from stored ones"""
    ...
    def find_best_match_with_index(
        ...
        """
        Find the best matching embedding from stored ones and return its index
        """
        if not stored_embeddings:
    ...
    async def execute(self, username: str, input_embedding: List[float]) -> dict:
        """
        Verify if the input embedding matches any stored embedding for the username
        Returns user info if match found, otherwise raises ValueError
        """
...
class VerifyUserWithImageUseCase:
    """Verify user by username and face image"""
    ...
    async def execute(self, username: str, image_bytes: bytes) -> dict:
        """
        Verify user by extracting embedding from image and comparing with stored ones
        """
        try:
            import cv2
            import numpy as np
